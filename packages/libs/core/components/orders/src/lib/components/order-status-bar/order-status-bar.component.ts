...
export class OrderStatusBarComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes && changes["order"]?.currentValue) {
      this.barNumberWidth = this.calculateWidth(changes["order"]?.currentValue?.orderStatus);
    }
  }

  private calculateWidth(orderStatus: OrderStatus) {
    const x = orderStatus === OrderStatus.ORDER_NEW ? 0 :
      orderStatus === OrderStatus.ORDER_ACCEPTED ? 1 :
        orderStatus === OrderStatus.ORDER_PROCESSING ? 2 : 4;
    return (x * 2 + 1) / 8;
  }
